...
export function UsersPageContent({
  homeHref = "/",
  excludeInvestors = false,
}: {
  homeHref?: string;
  /** В рассрочке инвесторы живут отдельно — в «Команде» их не показываем. */
  excludeInvestors?: boolean;
}) {
  ...
  }, [currentUser, router, homeHref]);

  async function loadUsers() {
    setLoading(true);
    try {
      const items = await usersApi.list();
      setUsers(excludeInvestors ? items.filter((item) => item.role !== "investor") : items);
    } finally {
      setLoading(false);
    }
  }
  ...
  }, [currentUser?.role, excludeInvestors]);
  ...
  return (
    <div className="page-stack">
      ...
      <div className="grid gap-2 sm:grid-cols-3">
        <StatCard label="Всего в команде" value={users.length} tone="brand" />
      ...
      <Card>
        <SectionTitle title="Список команды" />
        {loading ? (
          <LoadingState text="Загрузка команды..." />
        ) : users.length === 0 ? (
          <EmptyState
            action={
