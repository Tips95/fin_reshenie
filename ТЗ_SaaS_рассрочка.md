...
- [x] Дашборд owner: сейчас в кассе, ещё ожидается, осталось расходов, прогноз на конец месяца
- [x] Три раздела: Касса (движение денег), Портфель (долг клиентов), Итоги месяца (прибыль)
- [x] Касса («кубышка»): ручной остаток на начало месяца + перенос на следующий
- [x] Касса считается по факту: плановые статьи вычитаются только после отметки об оплате
      (`expense_payments`), прибыль при этом считает расходы по бюджету — расхождение равно
      сумме неоплаченных статей и подписано прямо на дашборде
- [x] Гражданские дела входят в кассу, но не в прибыль по банкротству
